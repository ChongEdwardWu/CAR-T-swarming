#!/usr/bin/env Rscript
# CAR T scRNA-seq — Step 1: Integration & initial clustering (Seurat SCT)
...
SAMPLES_ENV           <- Sys.getenv("SAMPLES", unset = "PostCAR,PostCC5")
INCLUDE_DATE_IN_NAMES <- as.logical(Sys.getenv("INCLUDE_DATE_IN_NAMES", unset = "FALSE"))
RESULTS_BASENAME      <- Sys.getenv("RESULTS_BASENAME", unset = "01_Integration_CART")
...
RES_CHOSEN <- as.numeric(Sys.getenv("RES_CHOSEN", unset = "0.6"))
...
# If you later define `seu_filt`, we'll visualize it; otherwise we skip gracefully.
if (exists("seu_filt")) {
  p_filt <- DimPlot(seu_filt, reduction = "umap", group.by = "CellType_l1", label = TRUE) + coord_fixed(ratio = 1)
  ggsave(file.path(FIG_DIR, "Step1_umap_filtered.png"), p_filt, width = 160, height = 140, units = "mm", dpi = 150)
}
