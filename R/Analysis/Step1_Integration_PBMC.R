#!/usr/bin/env Rscript
# PBMC scRNA-seq — Step 1: Integration & initial clustering (Seurat SCT)
...
SAMPLES_ENV           <- Sys.getenv("SAMPLES", unset = "PBMC_CAR,PBMC_CC5")
INCLUDE_DATE_IN_NAMES <- as.logical(Sys.getenv("INCLUDE_DATE_IN_NAMES", unset = "FALSE"))
RESULTS_BASENAME      <- Sys.getenv("RESULTS_BASENAME", unset = "01_Integration_PBMC")
RES_CHOSEN            <- as.numeric(Sys.getenv("RES_CHOSEN", unset = "1"))
...
# Parse sample list (defaults to two PBMC samples)
...
# Optional: set a specific reference sample by name via SEURAT_REF
SEURAT_REF <- Sys.getenv("SEURAT_REF", unset = "")
...
# Build exclusion (histone/mito/ribo/etc.) using RNA assay rownames if present
...
ggsave(file.path(FIG_DIR, "PBMC_Step1_elbow_plot.png"), elbow_plot, width = 140, height = 120, units = "mm", dpi = 150)

# UMAP/Neighbors (dims 1:30 per your original script)
seu <- RunUMAP(seu, reduction = "pca", dims = 1:30)
seu <- FindNeighbors(seu, reduction = "pca", dims = 1:30)

# Quick UMAP by group (if `group` meta exists)
umap_by_group <- try({ DimPlot(seu, reduction = "umap", group.by = c("group"), label.size = 2) }, silent = TRUE)
if (!inherits(umap_by_group, "try-error"))
  ggsave(file.path(FIG_DIR, "PBMC_Step1_umap_by_group.png"), umap_by_group, width = 160, height = 140, units = "mm", dpi = 150, bg = "white")
...
for (cn in meta_cols) {
  k <- seu@meta.data[[cn]]
  levels(k) <- as.character(seq_len(nlevels(k)))  # avoid level "0"
  seu@meta.data[[cn]] <- k
}
...
umap_split <- try({
  DimPlot(seu, reduction = "umap", split.by = "group",
          group.by = paste0("integrated_snn_res.", RES_CHOSEN), label = TRUE) + coord_fixed(ratio = 1)
}, silent = TRUE)
if (!inherits(umap_split, "try-error"))
  ggsave(file.path(FIG_DIR, sprintf("PBMC_Step1_umap_split_res%s.png", RES_CHOSEN)), umap_split,
...
if (length(qc_feats)) {
  ...
  ggsave(file.path(FIG_DIR, "PBMC_Step1_qc_vln.png"), vln, width = 200, height = 140, units = "mm", dpi = 150, bg = "white")
}

# -------------------- Summary tables (xlsx) -------------------------------
# Cell-type label column may differ; try common fields.
ctype_col <- intersect(c("CellType_immgen", "predicted.celltype.l1", "predicted.annotation"), colnames(seu@meta.data))[1]
...
if (!is.na(ctype_col)) {
  cluster_annot <- tibble(Cluster = seu$seurat_clusters, Source = seu$group, CellType = seu@meta.data[[ctype_col]]) %>%
    ...
    arrange(Cluster, dplyr::desc(perc)) %>% top_n(n = 5, wt = perc)
}
...
wb_save(wb, file.path(RES_DIR, "Step1_cluster_celltype.xlsx"), overwrite = TRUE)

# -------------------- (Optional) filtering placeholder --------------------
if (exists("seu_filt")) {
  p_filt <- DimPlot(seu_filt, reduction = "umap", group.by = "CellType_l1", label = TRUE) + coord_fixed(ratio = 1)
  ggsave(file.path(FIG_DIR, "PBMC_Step1_umap_filtered.png"), p_filt, width = 160, height = 140, units = "mm", dpi = 150)
}
